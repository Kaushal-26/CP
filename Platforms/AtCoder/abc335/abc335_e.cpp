...
void solve() {
        ...
        for(int i = 1; i <= n; i++) {
                ...
                clog << db(i) << db(g[i]) << db(ng[i]) << db(d.leader(i)) << db(indg[i]) << endl;
        }
        ...
        for(int i = 1, e; i <= n; i++) {
                e = d.leader(i);
                if(e != i || indg[e] != 0) continue;
                if(e == d.leader(1)) dis[e] = 1;
                Q.push(e);
        }
        ...
        while(!Q.empty()) {
                ...
                clog << db(cur) << db(dis[cur]) << endl;

                for(auto &u : ng[cur]) {
                        if(dis[cur] > 0) {
                                dis[u] = max(dis[u], dis[cur] + 1);
                        }
                        indg[u]--;
                        if(indg[u] == 0) {
                                Q.push(u);
                                if(d.leader(1) == u) dis[u] = 1;
                        }
                }
        }
