...
void solve() {
        ...
        FOR(i, n + 2) tot.insert(i);

        FOR(rep, n) {
                cout << (int)cur.size() << " ";
                REP(i, cur) tot.erase(i);

                set<int> ncur;
                REP(i, cur) {
                        auto it = tot.lower_bound(i);
                        ncur.insert(*it);
                        it--;
                        ncur.insert(*it);
                }
                ...
                REP(i, ncur) {
                        ...
                        auto it = tot.lower_bound(i + 1);
                        auto ti = prev(tot.lower_bound(i));
                        if(a[*it] + a[*ti] > d[i]) cur.insert(i);
                }
        }
