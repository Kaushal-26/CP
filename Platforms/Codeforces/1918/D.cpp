...
void solve() {
        ...
        auto pf = a;
        FOR(i, 1, n) pf[i] += pf[i - 1];
        
        set<pair<int, int>> ok;
        ok.insert({0, 0});

        auto dp = V<array<int, 2>>({0, 0}, n + 1);
        FOR(i, n) {
                int L = 1, R = pf[i], ans = R;
                while(L <= R) {
                        int mid = (L + R) / 2;
                        auto it = ok.lower_bound({mid, 0});
                        if(it != ok.end()) {
                                int id = it -> second;
                                if(dp[id - 1][0] <= mid && a[id] + dp[id - 1][1] <= mid) {
                                        ans = mid;
                                        R = mid - 1;
                                } else {
                                        L = mid + 1;
                ...
                dp[i] = {pf[i], 0};
                auto it = ok.lower_bound({pf[i] - ans, 0});
                if(it )
        }
}
