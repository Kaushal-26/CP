...
S op(S L, S R) {
        ...
        for(auto &el : R.ids) L.ids.push_back(el);
        ranges::sort(R.ids);
        return L;
}

S e() {return S{(int)1e18, vector<int>(0)};}
...
bool solve(int st, int en, int cur) {
        if(st > en) return 1;

        auto ok = seg.prod(st, en + 1);

        if(ok.mn != cur) {
                return 0;
        }

        if(ok.mn == 0 && ok.ids.size() > 1) {
                return 0;
        }

        bool pos = true;
        for(int i = 0; i < (int)ok.ids.size(); i++) {
                pos &= solve(st, ok.ids[i] - 1, cur + 1);
                st = ok.ids[i] + 1;
        }
        pos &= solve(st, en, cur + 1);
...
void solve() {
        ...
        for(int i = 0; i < n; i++) {
                cin >> a[i];
                r.push_back({a[i], vector<int>(1, i)});
        }

        seg = segtree<S, op, e>(r);

        solve(0, n - 1, 0) ?  cout << "YES\n" : cout << "NO\n";
}
